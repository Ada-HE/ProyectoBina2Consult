...
const FormularioContacto = () => {
  ...
  const obtenerDatosContacto = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/contacto/ver');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (idRegistro) {
        // Actualizar
        await axios.put(`http://localhost:4000/api/contacto/actualizar/${idRegistro}`, datos, {
          headers: {
        ...
        // Registrar
        await axios.post('http://localhost:4000/api/contacto/registrar', datos, {
          headers: {
            'CSRF-Token': csrfToken,
            'Content-Type': 'application/json',
          },
