...
const FormularioEslogan = () => {
  ...
  const obtenerEslogan = async () => {
    try {
      const response = await axios.get('https://localhost:4000/api/eslogan', { withCredentials: true });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post('https://localhost:4000/api/eslogan/actualizar', { eslogan: nuevoEslogan }, {
        headers: {
