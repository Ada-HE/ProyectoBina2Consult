...
const ActualizarIntentos = () => {
  ...
  useEffect(() => {
    ...
    const obtenerMaxIntentosActual = async () => {
      try {
        const response = await axios.get('https://localhost:4000/api/obtener-max-intentos');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://localhost:4000/api/cambiar-max-intentos',
        { maxIntentos },
        {
          headers: {
      ...
      const maxIntentosActualizado = await axios.get('https://localhost:4000/api/obtener-max-intentos');
