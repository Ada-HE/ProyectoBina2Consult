...
const FormularioRedesSociales = () => {
  ...
  const obtenerRedesSociales = async () => {
    try {
      const response = await axios.get('https://localhost:4000/api/redSocial', { withCredentials: true });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post('https://localhost:4000/api/redSocial/crear', nuevaRedSocial, {
        headers: {
  ...
  const guardarEdicion = async () => {
    ...
    try {
      await axios.put(`https://localhost:4000/api/redSocial/editar/${redSocialEditada.id}`, redSocialEditada, {
