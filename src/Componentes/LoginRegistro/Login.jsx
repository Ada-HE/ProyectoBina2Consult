  ...
  useEffect(() => {
    const verificarAutenticacion = async () => {
      try {
        ...
        const response = await fetch('https://localhost:4000/api/verificar-autenticacion', {
          method: 'GET',
          credentials: 'include',
        });
    ...
    try {
      if (!showMfa) {
        ...
        if (response.ok && data.requireMfa) {
          const qrResponse = await fetch(`https://localhost:4000/api/mfa/setup/${correo}`, {
            method: 'GET',
          });
        ...
        const response = await fetch('https://localhost:4000/api/mfa/verify', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'CSRF-Token': csrfToken,
